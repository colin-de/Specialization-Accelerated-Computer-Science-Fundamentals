...
void LinkedList<T>::insertOrdered(const T& newData) {
  // -----------------------------------------------------------
  // TODO: Your code here!
  Node* dataNode = new Node(newData);
  Node* setHead = head_;
  Node* setTail = tail_;
  ...
  if  (head_ == NULL && tail_ == NULL) {
    ...
    //newData at the beginning
    if  (dataNode->data <= setHead->data)
    {
      size_++;
      setHead->prev = dataNode;
      dataNode->next = setHead;
      head_ = dataNode;
    }
    
    //newData at the end
    else if (dataNode->data > setTail->data){
      size_++;
      setTail->next = dataNode;
      dataNode->prev = setTail;
      tail_ = dataNode;
    }  
    
    else if (setHead->next != NULL){
      while (setHead->next != NULL){
        if  (dataNode->data < setHead->data ){
          size_++;
          dataNode->next = setHead;
          setHead->prev->next = dataNode;
          dataNode->prev = setHead->prev;
          setHead->prev = dataNode;
        }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // -----------------------------------------------------------
  // TODO: Your code here!
  int totalSize = left.size_ + right.size_;
  int set = 0;
  ...
  while (set < totalSize){
    ...
    else if (left.empty() == 0 && right.empty() == 1){
